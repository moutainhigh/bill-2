...
public class DiscAccountService implements IAccountFacadeService {
   ...
   private ICoreRemoteService coreRemoteService;
    ...
    public AccountResponseDTO account(AccountRequestDTO accountReq)
        ...
        DiscApplyInfo apply=(DiscApplyInfo) accountReq.getApply();
        List<DiscBillInfo> billList=accountReq.getBillList();
...
//        af.setTransName(transName);//TODO:记账可能需要根据产品类型 来判断对应的交易编码
        ...
        double totAmt=0.0;
        for(Iterator<DiscBillInfo> iter=billList.iterator();iter.hasNext();){
            DiscBillInfo bill=iter.next();
...
//            afi.setDiscrepancyInterest()
            afi.setInfoId(bill.getDiscmxId());
            afi.setProdNo(af.getProdNo());
            afi.setRgctId(bill.getRgctId());
            afi.setSettlementAmt(bill.getDraftPayMoney());
            afi.setSettlementIntrst(MathScaleUtil.add(bill.getSalerInterest(), bill.getBuyerInterest()));
            ...
            balance.setInfoId(bill.getDiscmxId());
