...
public class MathScaleUtil {
	...
	/**
	 * 取绝对值和2位精度后，value在数字上小于、等于或大于baseValue时，返回-1、0或1
	 * 
	 * @param value
	 * @param baseValue
	 * @return
	 */
	public static int absCompareTo(double value, double baseValue) {
		if (value < 0) {
			value = -value;
		}
		if (baseValue < 0) {
	 ...
	 * @return
	 */
	public static int absCompareTo(String value, String baseValue) {
		if (value.charAt(0) == '-') { //如果第一个字符为-，则删除
			value = value.substring(1);
		}
		if (baseValue.charAt(0) == '-') { //如果第一个字符为-，则删除
