/********************************************
* 文件名称: AcctBalance.java
* 系统名称: 合融基础技术平台V2.0
* 模块名称:
* 软件版权: 北京合融科技有限公司
* 功能说明: 
* 系统版本: 2.0.0.1
* 开发人员: 技术平台组
* 开发时间:  
* 审核人员:
* 相关文档:
* 修改记录: 修改日期    修改人员    修改说明
*********************************************/
package com.herongtech.console.domain.acct.bean;


public class AcctBalance{
	private String balanceId = " ";	//主键
	private String infoId = " ";	//清单id
	private String rgctId = " ";	//登记中心id
	private String billNo = " ";	//票号
	private String startDt = " ";	//开始日期
	private String endDt = " ";	//结束日期
	private String custNo = " ";	//客户号/行号
	private String custName = " ";	//客户名称/行名
	private String prodNo = " ";	//产品号
	private String status = " ";	//状态
	private String createDt = " ";	//创建日期
	private String createTm = " ";	//创建时间
	private String updateDt = " ";	//更新日期
	private String updateTm = " ";	//更新时间
	private String storageBrchNo = " ";	//实物库存机构
	private String storageBrchName = " ";	//实物库存机构名称
	private String acctBrchNo = " ";	//账务库存机构
	private String acctBrchName = " ";	//账务库存机构名称
	public String getBalanceId(){
		return balanceId;
	}
	public void setBalanceId(String balanceId){
		this.balanceId = balanceId;
	}

	public String getInfoId(){
		return infoId;
	}
	public void setInfoId(String infoId){
		this.infoId = infoId;
	}

	public String getRgctId(){
		return rgctId;
	}
	public void setRgctId(String rgctId){
		this.rgctId = rgctId;
	}

	public String getBillNo(){
		return billNo;
	}
	public void setBillNo(String billNo){
		this.billNo = billNo;
	}

	public String getStartDt(){
		return startDt;
	}
	public void setStartDt(String startDt){
		this.startDt = startDt;
	}

	public String getEndDt(){
		return endDt;
	}
	public void setEndDt(String endDt){
		this.endDt = endDt;
	}

	public String getCustNo(){
		return custNo;
	}
	public void setCustNo(String custNo){
		this.custNo = custNo;
	}

	public String getCustName(){
		return custName;
	}
	public void setCustName(String custName){
		this.custName = custName;
	}

	public String getProdNo(){
		return prodNo;
	}
	public void setProdNo(String prodNo){
		this.prodNo = prodNo;
	}

	public String getStatus(){
		return status;
	}
	public void setStatus(String status){
		this.status = status;
	}

	public String getCreateDt(){
		return createDt;
	}
	public void setCreateDt(String createDt){
		this.createDt = createDt;
	}

	public String getCreateTm(){
		return createTm;
	}
	public void setCreateTm(String createTm){
		this.createTm = createTm;
	}

	public String getUpdateDt(){
		return updateDt;
	}
	public void setUpdateDt(String updateDt){
		this.updateDt = updateDt;
	}

	public String getUpdateTm(){
		return updateTm;
	}
	public void setUpdateTm(String updateTm){
		this.updateTm = updateTm;
	}

	public String getStorageBrchNo(){
		return storageBrchNo;
	}
	public void setStorageBrchNo(String storageBrchNo){
		this.storageBrchNo = storageBrchNo;
	}

	public String getStorageBrchName(){
		return storageBrchName;
	}
	public void setStorageBrchName(String storageBrchName){
		this.storageBrchName = storageBrchName;
	}

	public String getAcctBrchNo(){
		return acctBrchNo;
	}
	public void setAcctBrchNo(String acctBrchNo){
		this.acctBrchNo = acctBrchNo;
	}

	public String getAcctBrchName(){
		return acctBrchName;
	}
	public void setAcctBrchName(String acctBrchName){
		this.acctBrchName = acctBrchName;
	}


}
