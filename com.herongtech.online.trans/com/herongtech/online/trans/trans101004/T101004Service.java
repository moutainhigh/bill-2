...
import com.herongtech.online.trans.trans101004.Var101004Bean;
import com.herongtech.online.trans.trans101004.Var101004Result;
 ...
 *  提示收票待签收票据查询
 */
public class T101004Service extends OnlineBaseService {
	...
	protected void action(Context context) throws Exception {
		Var101004 transVar = new Var101004();
	 ...
	 */
	protected void CheckData(Context context, Var101004 Var101004) throws BizAppException{
String request = (String)ContextUtil.getRequestData(context);
		
		Map<String, Class> clazzMap = new HashMap<String, Class>();
        clazzMap.put("Document", Var101004.class);
        Var101004 temp=(Var101004)XmlBeanUtil.xml2Bean(clazzMap, request);
		if (StringUtils.isEmpty(temp.getCustAccount())){
			throw new BizAppException(IErrorNo.ERR_CUSTACCOUNTNULL, "客户账号未上送");
		}
		org.springframework.beans.BeanUtils.copyProperties(temp, Var101004);
	 ...
	 * @throws BizAppException
	 */
    protected void ToLocal(Context context, Var101004 Var101004) throws BizAppException{
    	RcBaseSearchBean rcbean = new RcBaseSearchBean();
    	rcbean.setBillClass("2");
    	rcbean.setHoldAcctNo(Var101004.getCustAccount());
    	try {
    	
    	IDB session = DBFactory.getDB();
    	int startIndex = Var101004.getCurrentPage();//记录开始值
    	...
    	List<Var101004Bean> var101004beanlist = new ArrayList<Var101004Bean>();
    	rgctbilllist = session.getBeanListByListForPage(sql, RgctBill.class, startIndex, Var101004.getPageSize(), rgctlist);
			
    	for (int i = 0; i < rgctbilllist.size(); i++) {
    		session.beginTransaction();
 	    	Var101004Bean bean = new Var101004Bean();
 	    	...
 	    	var101004beanlist.add(bean);
 	    	session.endTransaction();
 		}	
//    	if(resultData.getRowCount() <= 0){
//    		throw new BizAppException(IErrorNo.ERR_DBERR, "");
//    	}
    	Var101004.setBeanlist(var101004beanlist);
    	Var101004.setRetNum(var101004beanlist.size());
			Var101004.setTotRecNum(
     ...
     * @throws BizAppException
     */
    protected void PackAnswer(Context context, Var101004 Var101004) throws BizAppException{
//    	IData request = (IData)ContextUtil.getRequestData(context);
//		AnswerData response = new AnswerData(request);
//		
//		IData resultData = Var101004.getResultData();
//		resultData.beforeFirst();
//		for(int i = 1; i <= resultData.getRowCount(); i++){
//			resultData.next();
//			if(i == 1){
//				response.setValue(i, IFieldName.totalRows, Var101004.getTotRecNum());
...
//		ContextUtil.setResponseData(context, response.getAnswerData());
    	 ...
    	 List<Var101004Bean> infoList=Var101004.getBeanlist();

    	 
    	 Var101004Result proResult=new Var101004Result();
    	 proResult.setType("S");
    	 proResult.setExSerial(trans.getExSerial());
    	 proResult.setFunctionId(trans.getFunctionId());
    	 proResult.setCurrentPage(Var101004.getCurrentPage());//当前页
    	 proResult.setTotalRows(Var101004.getTotRecNum());//总条数
    	 proResult.setRetNum(Var101004.getRetNum());//返回条数
         proResult.setBeanlist(infoList);
               
         Map<String, Class> clazzMap = new HashMap<String, Class>();
         clazzMap.put("Document", Var101004Result.class);
         clazzMap.put("info", Var101004Bean.class);
