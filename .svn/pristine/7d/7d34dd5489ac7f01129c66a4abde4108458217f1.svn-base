<%-- 
 * 文件名称: sale_cancel_audit_batch_list.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 待审核的批次列表页面
	...
	<div class="clearfix">
		<div id="page-content" class="page-content">
				...
				<form id="btnForm">
					<div class="row-fluid">
						<div class="span6" id="btn-left">
							<a class="btn-mini" onclick="auditDetailList();">下一步</a>
						</div>
					</div>
				</form>
			</div>
			...
			<div id="page" class="pagination">
				<form action="<%=basePath%>saleAuditController.do?method=cancelAuditBatchList"
...
<script>
	function auditDetailList(){
		...
		dataCollectForm.action = "<%=basePath%>saleAuditController.do?method=cancelAuditBillManage";
	...
	function goApplyInfo(saleId){
		 ...
		 diag.Drag = true;
		 diag.Title ="详情";
		 diag.URL = '<%=basePath%>saleApplyController.do?method=goApplyInfo&saleId='+saleId+'&flag=cancelAudit';
		 diag.Width = 1000;
		 diag.Height = 530;
