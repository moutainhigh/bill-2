<%-- 
 * 文件名称:disc_elec_cancel_apply_bill_list.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
...
<body>
	<div class="clearfix">
		<div id="page-content" class="page-content">
			...
			<%-- 按钮操作区 --%>
			<div id="center">
				<form id="btnForm" name="btnForm" action="<%=basePath%>discApplyController.do?method=cancelElecBatch" method="post">
					<div class="row-fluid">
		...
		<div id="page" class="pagination">
			<form action="<%=basePath%>discApplyController.do?method=cancelBillList"  name="pageForm" method="post">
...
if(buyPayRate=='3'){
	...
	function cancel(){
		...
		if(result){
				...
				$.ajax({
					type: "POST",
					url: '<%=basePath%>discApplyController.do?method=doCancelApply',
					...
					success: function(data){	
						if (data.success){  //处理成功
						   if(data.attributes.count == "0"){
						    ...
						    dataCollectForm.action = "<%=basePath%>discApplyController.do?method=cancelElecBatch";
		                    dataCollectForm.submit();
						   }else{
							dynamicHiddenElement('dataCollectForm','acctNo',custAccount);
		                    dynamicHiddenElement('dataCollectForm','discId',discId);
		                    modal("#layer_loading,#image");
		                    dataCollectForm.action = "<%=basePath%>discApplyController.do?method=cancelBillList";
		                    dataCollectForm.submit();
						   }
						} else {
							bootbox.alert('撤销失败!');
	...
	function goHistory(){
		modal("#layer_loading,#image");
		btnForm.action = "<%=basePath%>discApplyController.do?method=cancelElecBatch";
		btnForm.submit();
	}
	//详情页面
