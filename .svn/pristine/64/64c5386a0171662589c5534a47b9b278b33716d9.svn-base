...
<div class="clearfix">
	<div id="jump-content" class="page-content">
		<form action="<%=basePath%>intoAuditController.do?method=elecAudit" method="post" name="Form" id="Form" class="form-search">
			<div class="row-fluid">
				<label class="text-right" for="status"><span class="star">*</span>审核状态</label>
					<t:dictSelect valid="required" name="status" other="" dictGroup="K_AUDIT_STATUS" haveHead="true" title="审核状态" >
					</t:dictSelect>
			</div>
			<div class="row-fluid">
			...
			<div class="row-fluid into">
				<div class="col-xs-12 col-sm-4 col-md-4 center">
					<a class="btn btn-x" onclick="save();">保存</a>
	...
	function save(){
		if($("#Form").valid()){
			if($("#status").val()=="0" && $("#option").val()==""){
				bootbox.alert("请输入审批意见");
				return;
			}else{
				modal("#layer_loading,#image");
				$.ajax({
					url:"<%=basePath%>intoAuditController.do?method=elecAudit",
					type:"POST",
					data:$("#Form").serialize(),
					dataType:"json",
					success:function(rs){
