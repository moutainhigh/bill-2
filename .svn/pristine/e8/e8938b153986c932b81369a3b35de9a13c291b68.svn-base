	...
	<div class="clearfix">
				...
				<form  id="btnForm">
					<div class="row-fluid">
						<div class="span6" id="btn-left">
							<a class="btn-mini" onclick="add('${query.intoId}','${batch.batchType}','${batch.prodNo}');">录入</a>
							<a class="btn-mini" onclick="doImport('${query.intoId}','${batch.batchType}');">批量录入</a>
							<a class="btn-mini" onclick="copyAdd('${query.intoId}','${batch.batchType}','${batch.custNo}','${batch.custName}');">复制录入</a>
						...
						<c:choose>
							<c:when test="${not empty resultList}">
								<c:forEach items="${resultList}" var="batchList" varStatus="vs">
									<tr>
										<td class="center"><input type="checkbox" name="ids" value="${batchList.intomxId}"/></td>
										...
										<td class="center"><a href="javascript:goBillDetail('${batchList.intomxId}')">查看</a></td>
									</tr>
								</c:forEach>
							</c:when>
							<c:otherwise>
		...
		<div>
			<div id="page" class="pagination">
					<form action="<%=basePath%>intoApplyController.do?method=billManage"  name="pageForm" method="post">
					<%@ include file="/webpage/system/admin/pageFormContent.jsp"%>
					<input type="hidden" name="intoId" value="${intoId}"/>
					<input type="hidden" name="prodNo" id = "prodNo" value="${batch.prodNo}"/>
					<input type="hidden" name="intomxId" value="${batchList.intomxId}"/>
				</form>
			</div>
		</div>
	</div>
...
<script>
	...
	function goHistory(){
        ...
        dataCollectForm.action="<%=basePath%>intoApplyController.do?method=searchBatch";
    ...
    //录入
    function add(intoId,batchType,prodNo){
	   	 var diag = new top.Dialog();
		 diag.Drag = true;
		 diag.Title ="录入";
		 diag.URL = "<%=basePath%>intoApplyController.do?method=toAddBill&action=intoApplyController.do?method=saveBill&intoId="+intoId+"&batchType="+batchType+"&prodNo="+prodNo;
		 diag.Width = 800;
		 diag.Height =500;
		 diag.CancelEvent = function(){ //关闭事件
			pageForm.submit();
	...
	function edit(batchType){
		...
		var id = getCheckStr("ids");
		 ...
		 diag.Drag = true;
		 diag.Title ="修改";
		 diag.URL = "<%=basePath%>intoApplyController.do?method=toEditBill&action=intoApplyController.do?method=saveBill&intomxId="+id+"&batchType="+batchType;
		 diag.Width = 800;
		 diag.Height =500;
		 diag.CancelEvent = function(){ //关闭事件
			 pageForm.submit();
	...
	function del(){
		...
		bootbox.confirm("确定要删除选中的记录吗?", function(result) {
			if(result) {
				...
				pageForm.action = "<%=basePath%>intoApplyController.do?method=delBill";
				pageForm.submit();
			}
		});
	}
	...
	function apply(custNo,custName){
		...
		var ids = getCheckStr("ids");
		 modal("#layer_loading,#image");//点击票据管理按钮，弹出‘加载中’的遮罩层
		$.ajax({
			url:"<%=basePath%>intoApplyController.do?method=apply",
			type:"POST",
			data:{"ids":ids},
			dataType:"json",
			success:function(rs){
	...
	//复制录入
	function copyAdd(intoId,batchType,custNo,custName){
		var checkNum = getCheckNum("ids");
		if (checkNum != 1){
		...
		var diag = new top.Dialog();
		 diag.Drag = true;
		 diag.Title ="复制录入";                 															
		 diag.URL = "<%=basePath%>intoApplyController.do?method=toCopyBill&intoId="+intoId+"&billType="+batchType+"&intomxId="+id+"&custNo="+custNo+"&custName="+custName+"&prodNo="+prodNo;
		 diag.Width = 800;
		 diag.Height =500;
		 diag.CancelEvent = function(){ //关闭事件
			pageForm.submit();
	...
	function doImport(discId,billType){
		...
		diag.Drag = true;
		diag.Title ="导入结果";                 															
		diag.URL = "<%=basePath%>webpage/busi/common/import_result.jsp?batchId="+discId+"&billType="+billType+"&serviceName=com.herongtech.console.service.busiservice.into.IntoImportDataService";
		diag.Width = 1500;
		diag.Height =500;
		diag.CancelEvent = function(){ //关闭事件
			pageForm.submit();
