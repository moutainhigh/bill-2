<%-- 
 * 文件名称: revoke_bill_details.jsp.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 批次下清单列表
...
<body>
	<div class="clearfix">
		<div id="page-content" class="page-content">
			<div id="header">
				<%-- 批次信息明细  --%>
				<form action="<%=basePath%>subcollApplyController.do?method=revokeBillDetailsManage" id="page-content" method="post" name="subcollManage" id="subcollManage" class="form-search">
					<div class="row-fluid">
				...
				<div>
					<div id="page" class="pagination">
						<form action="<%=basePath%>subcollApplyController.do?method=revokeBillDetailsManage"  name="pageForm" method="post">
...
<script>
	...
	function goHistory(){
		...
		dataCollectForm.action = "<%=basePath%>subcollApplyController.do?method=revokeBatchManage";
	...
	function rev(){
		...
		modal("#layer_loading,#image");//点击票据管理按钮，弹出‘加载中’的遮罩层
		dataCollectForm.action = "<%=basePath%>subcollApplyController.do?method=revokeBillForApply";
		dataCollectForm.submit();
	}
		//详情页面
