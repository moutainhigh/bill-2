			...
			<div id="header">
				<form action=""	method="post" name="userForm" id="userForm" class="form-search detail-form">
					...
					<div class="row-fluid">
						...
						<input type="text" class="input-medium" name="custName" value="${batch.custName}" readonly />
				...
				<div>
					<div id="page" class="pagination">
							<form action="<%=basePath%>collateralizationStorageController.do?method=storageBillList"  name="pageForm" method="post">
							<%@ include file="/webpage/system/admin/pageFormContent.jsp"%>
							<input type="hidden" name="saveId" value="${batch.saveId}"/>
						</form>
					</div>
				</div>
			</div>
...
<script>
	...
	function goHistory(){
        ...
        dataCollectForm.action="<%=basePath%>collateralizationStorageController.do?method=storageBatchList";
	...
	function doStorage(){
		var checkNum = getCheckNum("ids");
	   	...
	   	var savemx_ids = getCheckStr("ids");
	   	var saveId = "${saveId}";
	   	bootbox.confirm("确定要入库选中的记录吗?", function(result) {
			if(result) {
				...
				$.ajax({
					type: "POST",
					url: '<%=basePath%>collateralizationStorageController.do?method=storage',
			    	data: {'savemx_ids': savemx_ids,'saveId': saveId},
					dataType:'json',
					cache: false,
	...
	function printList(){
		...
		doPrint("addPrintController.do?method=doPrint&ids="+ids+"&moduleId=1110204&type=CollateralizationListPrint&baid="+batch_id+"&handleType=入 库"); 	 	
	}
</script>
</body>
</html>