<%--
 * 文件名称: saleback_entity_apply_bill_list.jsp
 * 系统名称: 票据管理平台
 * 模块名称     纸票返售申请清单页面
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
...
<body>
	<div class="clearfix">
		<div id="page-content" class="page-content">
			<div id="header">
				<form action="<%=basePath%>SalebackApplyController.do?method=billManage" method="post" name="batchForm" id="batchForm" class="form-search">
					...
					<input type="hidden" name="rebuyId" value="${bean.rebuyId}"/>
					<input type="hidden" name="isInner" value="${bean.isInner}"/>
					...
					<input type="hidden" id="iscancel" name="iscancel" value="1"/>
					<input type="hidden" id="rateids" name="rateids" value="1"/>
					...
					<div class="row-fluid firstLine">
						...
						<div type="text" class="pull-left batch">${batch.batchNo}</div>
					...
					<div class="row-fluid outer">
						...
						<input type="text" class="input-medium" id="inAcctNo" name="inAcctNo"  value="${batch.inAcctNo}" onblur="fillAcctInfo();" placeholder="请输入入账账号"/>
					...
					<div class="row-fluid threeLine">
						...
						<input class="input-medium input-date" name="createDt" id="createDt" value="${batch.createDt}" type="text"  placeholder="开始日期" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});"/></td>
						...
						<c:choose>
							<c:when test="${not empty billList}">
								<c:forEach items="${billList}" var="billList" varStatus="vs">
									<tr>
										<td class="center"><input type="checkbox" name="ids" value="${billList.salebackmxId}"/></td>
										...
										<td class="center">
											<a href="javascript:goDetail('${billList.salebackmxId}')">查看</a>
										</td>
						</tr>
						</c:forEach>
							</c:when>
			...
			<div>
				<div id="page" class="pagination">
					<form action="<%=basePath%>SalebackApplyController.do?method=billManage" method="post" name="pageForm">
						<%@ include file="/webpage/system/admin/pageFormContent.jsp"%> 
						<input type="hidden" name="rebuyId" id="rebuyId" value="${bean.rebuyId}" />
						<input type="hidden" name="billClass" id="billClass" value="${bean.billClass}" />
						<input type="hidden" name="isInner" id="isInner" value="${bean.isInner}" />
						<input type="hidden" name="rateids" id="rateids" value="${rateids}" />
						<input type="hidden" name="salebackId" id="salebackId" value="${bean.salebackId}" />
						<input type="hidden" name="newsalebackId" id="newsalebackId" value="${newsalebackId}" />
						<input type="hidden" name="iscancel" id="iscancel" value="${iscancel}" />
					</form>
				</div>
			</div>
		</div>
...
<script>
	...
	var result = false;
		...
		if(new Date(createdt)>=new Date(salebackduedt)){
			result=true;
		}
		 var checkNum = getCheckNum("ids");
		  var salebackmxids = getCheckStr("ids"); 
	 				 ...
	 				 $.ajax({
		            		...
		            		success:function(data){
		            			...
		            			bootbox.alert("请先试算利息");
					            	...
					            	modal("#layer_loading,#image");
					            	dynamicHiddenElement('dataCollectForm','isInner',"${bean.isInner}");
								  	dynamicHiddenElement('dataCollectForm','billClass',"1");
								  	dynamicHiddenElement('dataCollectForm','rebuyId',"${bean.rebuyId}");
					            	dynamicHiddenElement('dataCollectForm','salebackId',salebackId);
					            	 dynamicHiddenElement('dataCollectForm','salebackmxids',salebackmxids);
				     				 dataCollectForm.action = "<%=basePath%>SalebackApplyController.do?method=applysubmitentity";
					    			 dataCollectForm.submit();
		            			}
		            		},
		            		error:function(data){
	...
	if(!result){
		...
		var backDate=$("#createDt").val();
	     var checkNum = getCheckNum("ids");
	     var salebackId = "${batch.salebackId}";
	     var rebuyId=$("rebuyId").val();
	     var rate = $("#rate").val();//利率
	     if(!reg.test(backDate)|| backDate.length>10){
	     	 showTips("createDt","请输入有效的日期");
	     	 return false;
	     }
	     var inAcctNo = $("#inAcctNo").val();//入账账号
	     var salebackIdss = "${salebackIdss}";
	      var rateids = getCheckStr("ids"); //勾选的要计算利息的票据
	      document.getElementById("rateids").value=rateids;
	     if (checkNum == 0){
		...
		if(new Date(createdt)<new Date(salebackopendt)){
			showTips("createDt","返售日期必须大于返售开放日");
			return false;
		}
		var ids = getCheckStr("ids");
		 ...
		 diag.Drag = true;
		 diag.Title ="利息复核";
		 diag.URL = "<%=basePath%>SalebackApplyController.do?method=toElecInterestTrial&salebackmxId="+ids+"&salebackId="+salebackId+"&billClass="+billClass+"&rebuyId="+rebuyId+"&inAcctNo="+inAcctNo+"&rate="+rate+"&salebackIdss="+salebackIdss+"&createdate="+createdt;       
		 diag.Width = 480;
		 diag.Height = 200;
		 diag.CancelEvent = function(){ //关闭事件
			batchForm.submit();
	...
	function printList(){
		...
		$.ajax({
			url:"SalebackApplyController.do?method=isElecInterestTrial",
          		data:{"salebackmxids":ids,"salebackId":salebackId},
          		type:"post",
          		dataType:"json",
          		async: false,
          		success:function(data){
