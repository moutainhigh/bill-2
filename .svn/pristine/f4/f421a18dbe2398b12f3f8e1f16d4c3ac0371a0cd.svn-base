<%-- 
 * 文件名称: sale_cancel_endorse_batch_list.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 待审核的批次列表页面
	...
	<div class="clearfix">
		<div id="page-content" class="page-content">
			...
			<div id="page" class="pagination">
				<form action="<%=basePath%>saleEndorseController.do?method=queryForCancelSaleEndorse"
...
<%@ include file="/webpage/system/admin/footer.jsp"%>
	<script>
		function endorseDetailList(){
			...
			if (checkNum ==0){
		 		bootbox.alert("请先选择要进行撤销背书操作的批次记录");
		 		return;
		 	}else if (checkNum >1){
	  			bootbox.alert("一次只能对一个批次进行撤销背书操作");
	  			return;
	  	 	}
		   	var saleId = getCheckStr("saleId");
		   	dynamicHiddenElement('dataCollectForm','batchId',saleId);
		   	modal("#layer_loading,#image");//点击票据管理按钮，弹出‘加载中’的遮罩层
			dataCollectForm.action = "<%=basePath%>saleEndorseController.do?method=queryCancelBillList";
		...
		function goApplyInfo(saleId){
			 ...
			 diag.Drag = true;
			 diag.Title ="详情";
			 diag.URL = '<%=basePath%>saleApplyController.do?method=goApplyInfo&saleId='+saleId+'&flag=cancelEndorse';
			 diag.Width = 1000;
			 diag.Height = 505;
