<%-- 
 * 文件名称: sale_account_batch_list.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 批次列表页面
	...
	<form  action="#" name="dataCollectForm" method="post"></form>
	 ...
	 <div>
		<div id="page" class="pagination">
			<form action="<%=basePath%>saleAccountController.do?method=elecQueryApplyForSaleAcct"  name="pageForm" method="post">
...
<script>
	function billManage(){
		var checkNum = getCheckNum("saleId");
		 if (checkNum ==0){
		 	bootbox.alert("请先选择一条要进行记账操作的批次记录");
		 	return;
		 }else if (checkNum >1){
	  		bootbox.alert("一次只能对一个批次进行记账操作");
	  		return;
	  	 }
	  	 var saleId = getCheckStr("saleId");
	  	dynamicHiddenElement('dataCollectForm','batchId',saleId);
	  	modal("#layer_loading,#image");//点击票据管理按钮，弹出‘加载中’的遮罩层
		dataCollectForm.action = "<%=basePath%>saleAccountController.do?method=toSubmitElecSaleAccount";
	...
	function goApplyInfo(saleId){
		 ...
		 diag.Drag = true;
		 diag.Title ="详情";
		 diag.URL = '<%=basePath%>saleApplyController.do?method=goApplyInfo&saleId='+saleId+'&flag=elecAccount';
		 diag.Width = 1000;
		 diag.Height = 505;
