<%-- 
 * 文件名称: disc_cancel_audit_list.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
...
<body>
	<div class="clearfix">
		<div id="page-content" class="page-content">
			<div id="header">
				<form action="<%=basePath%>discAuditController.do?method=cancelAuditList"  method="post" id="page-content" name="custForm" style="padding:0px;" class="form-search">
   				<%-- 查询区  --%>
					<div class="row-fluid">
						...
						<a class="btn xs" id="search" onclick="doSearch();">查询</a>
					</div>
				</form>
			</div>
			<%-- 按钮操作区 --%>
			...
			<div id="page" class="pagination">
				<form action="<%=basePath%>discAuditController.do?method=cancelAuditList" method="post" name="pageForm">
...
<script>
	...
	function reviewList(){
		...
		dynamicHiddenElement('custForm','discId',discId);
		custForm.action="<%=basePath%>discAuditController.do?method=cancelAuditDetailList";
		custForm.submit();
	}
	//详情页面
