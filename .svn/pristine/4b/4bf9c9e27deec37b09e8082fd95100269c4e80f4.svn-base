<%-- 
 * 文件名称: disc_cancel_account_detail_list.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
...
<body>
	<div class="clearfix">
		<div id="page-content" class="page-content">
			<div id="header">
				<form action=""	method="post" name="Form" id="Form" class="form-search detail-form">
					...
					<div class="row-fluid">
						...
						<input type="text" class="input-medium" name="billTypes" readonly value="${fns:getDictLabel('K_BILL_TYPE',batch.billType)}"/>
					...
					<div class="row-fluid">
						...
						<label for="rate" class="pull-left control-label discLabeFirst">贴现利率</label>
						<input type="text" class="input-medium" name="rate" disabled value="${batch.rate}"/>
						<label for="totalNum" class="pull-left control-label discLabeSecond">成本利率</label>
			...
			<%-- 按钮操作区 --%>
			<div id="center">
				<form  id="btnForm" method="post" action="">
					<div class="row-fluid">
						<div class="span6" id="btn-left">
						</div>
						<div class="span6" id="btn-right">
							<a class="btn-mini pull-right" onclick="goHistory();">返回</a>
							<a class="btn-mini pull-right" href="javascript:cancelAccount();">撤销记账</a>
					   </div>
				 	 </div>
				</form>
			</div>
			<div id="footer" class="footer">
				<table class="table table-striped table-bordered table-hover" id="tab" cellpadding="0" cellspacing="0">
					<thead>
						<tr>
							<th class="center"><input type="checkbox" id="selectall" name="selectall" onclick="selectAll('selectall','ids')"/></th>
		...
		<div>
			<div id="page" class="pagination">
				<form action="<%=basePath%>discAccountController.do?method=cancelAccountDetailList" method="post" name="pageForm">
...
<script>
	...
	function cancelAccount(){
		var checkNum = getCheckNum("ids");
	   	if (checkNum <= 0){
	   		bootbox.alert("请先选择要撤销记账的记录");
	   		return;
	   	}
	   	var discmx_ids = getCheckStr("ids");	
	   	bootbox.confirm("确定要撤销选中的记录吗?", function(result) {
			if(result) {
				top.jzts();
				$.ajax({
					type: "POST",
					url: '<%=basePath%>discAccountController.do?method=cancelAccount',
			    	data: {'ids': discmx_ids},
					dataType:'json',
					success: function(data){	
						if (data.success){  //入库成功
							modal("#layer_loading,#image");//点击票据管理按钮，弹出‘加载中’的遮罩层
							Form.action = "<%=basePath%>discAccountController.do?method=cancelAccountList";
							Form.submit();
						} else {
							top.hangge();
							alert("撤销失败!"); 
