<%-- 
 * 文件名称: collate_cancel_apply_bill_list.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
				...
				<div id="footer" class="footer">
					...
					<div>
						<div id="page" class="pagination">
							<form action="<%=basePath%>collateralizationApplyController.do?method=cancelApplyBillList"  name="pageForm" method="post">
...
<script>
	...
	function goHistory(){
        ...
        dataCollectForm.action="<%=basePath%>collateralizationApplyController.do?method=cancelApplyBatchList";
	...
	function cancelApply(){
		...
		modal("#layer_loading,#image");//点击票据管理按钮，弹出‘加载中’的遮罩层
		$.ajax({
			url:"<%=basePath%>collateralizationApplyController.do?method=cancelApply",
			type:"POST",
			data:{"ids":ids,"saveId":saveId},
			dataType:"json",
			success:function(rs){
				if(rs.success){
					dataCollectForm.action = "<%=basePath%>collateralizationApplyController.do?method=cancelApplyBatchList";
