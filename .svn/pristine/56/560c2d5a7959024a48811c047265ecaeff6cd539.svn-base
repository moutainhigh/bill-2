<%-- 
 * 文件名称: disc_elec_audit_batch_list.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
...
<body>
	<div class="clearfix">
		<div id="page-content" class="page-content">
			<div id="header">
				<form action="<%=basePath%>discAuditController.do?method=auditListElec" method="post" id="page-content" name="searchForm" style="padding:0px;" class="form-search">
				<%-- 查询区  --%>
					<div class="row-fluid">
			...
			<div id="page" class="pagination">
				<form action="<%=basePath%>discAuditController.do?method=auditListElec"  name="pageForm" method="post">
...
<script>
		...
		function auditDetailList(){
		      var checkNum = getCheckNum("discId");
	   		 if (checkNum !=1){
	   			bootbox.alert("一次只能对一个批次进行审核");
	   			return;
	   	 	}
	   		var discId = getCheckStr("discId");	
	   		modal("#layer_loading,#image");
			location.href="<%=basePath%>discAuditController.do?method=auditBillList&discId="+discId;
		}
		//详情页面
