<%-- 
 * 文件名称: sale_endorse_bill_list.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 待审核批次的票据清单列表页面
...
<body>
	<div class="clearfix">
		<div id="page-content" class="page-content">
			...
			<%-- 按钮操作区 --%>
			<div id="center">
				<form id="btnForm" name="btnForm" action="<%=basePath%>saleEndorseController.do?method=queryBillList" method="post">
					<div class="row-fluid">
			...
			<div id="page" class="pagination">
				<form action="<%=basePath%>saleEndorseController.do?method=queryBillList"
...
<script>
	...
	function goHistory(){
		...
		dataCollectForm.action = "<%=basePath%>saleEndorseController.do?method=queryForSaleEndorse";
	...
	function endorse(){
		...
		modal("#layer_loading,#image");//点击票据管理按钮，弹出‘加载中’的遮罩层
		dataCollectForm.action = "<%=basePath%>saleEndorseController.do?method=submitSaleEndorse";
		dataCollectForm.submit();
	}
	//打印转贴现转出清单
		...
		$.ajax({
			...
			success: function(data){
  				if(!data.success){
					...
					doPrint("addPrintController.do?method=doPrint&ids="+ids+"&moduleId=1060301&type=SaleListPrint&baid="+batch_id+"&handleType=背 书"); 	 
				}
  			}
  		});	
	}
