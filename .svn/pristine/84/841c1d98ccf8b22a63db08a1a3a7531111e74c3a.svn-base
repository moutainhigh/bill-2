 ...
 * 
 */
public class T120005Service extends OnlineBaseService {
	...
	protected void action(Context context) throws Exception {
		Var120005 transVar = new Var120005();
	 ...
	 */
	protected void CheckData(Context context, Var120005 var120005) throws BizAppException{
		String request = (String)ContextUtil.getRequestData(context);
		Map<String, Class> clazzMap = new HashMap<String, Class>();
		clazzMap.put("Document", Var120005.class);
		Var120005 temp=(Var120005)XmlBeanUtil.xml2Bean(clazzMap, request);
		if (StringUtils.isEmpty(temp.getCustAccount())){
			throw new BizAppException(IErrorNo.ERR_CUSTACCOUNTNULL, "客户账号未上送");
		}
		BeanUtils.copyProperties(temp, var120005);
	}
	 ...
	 * @throws BizAppException
	 */
    protected void ToLocal(Context context, Var120005 var120005) throws BizAppException{
    	RcBaseSearchBean searchBean=new RcBaseSearchBean();
        searchBean.setHoldAcctNo(var120005.getCustAccount());
        searchBean.setBillClass(IDict.K_BILL_CLASS.K_ELEC_BILL);
        List<Var120005InfoBean> result =new ArrayList<Var120005InfoBean>();
		...
		try {
			rgctBill = session.getBeanListByListForPage(
					sql,
					RgctBill.class,
					Integer.valueOf(var120005.getCurrentPage()),
					Integer.valueOf(var120005.getPageSize()),
					list);
			var120005.setTotalRows(Integer.toString(session.account("select count(*) from ("+sql+")")));
		...
		if(rgctBill.size() <= 0){
    		...
    		for (RgctBill rgctBillList:rgctBill) {
    			Var120005InfoBean bean=new Var120005InfoBean();
     ...
     * @throws BizAppException
     */
    protected void PackAnswer(Context context, Var120005 var120005) throws BizAppException{
        ...
        List<Var120005InfoBean> infoList=var120005.getResult();
        Var120005Result result=new Var120005Result();
        result.setProResult(proResult);
        result.setResult(infoList);
        
        Map<String, Class> clazzMap = new HashMap<String, Class>();
        clazzMap.put("Document", Var120005Result.class);
        clazzMap.put("info", Var120005InfoBean.class);
