<%--
 * 文件名称: saleback_entity_cancel_acount_bill.jsp
 * 系统名称: 票据管理平台
 * 模块名称: 纸票返售撤销记账清单页面
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 批次下清单列表
		...
		<div id="page-content" class="page-content">
				...
				<form id="btnForm" name="btnForm" action="<%=basePath%>SaleBackAuditController.do?method=auditbilllist" method="post">
					<div class="row-fluid">
						...
						<div class="span6" id="btn-right"> 
							<a class="btn-mini pull-right" onclick="cancelaccount();">撤销记账</a>
			...
			<div id="footer" class="footer">
				<table class="table table-striped table-bordered table-hover"
					...
					<thead>
						<tr>
							<th class="center" onclick="selectAll('zcheckbox', 'salebackmxId')">
						...
						<c:choose>
							<c:when test="${not empty resultList}">
								<c:forEach items="${resultList}" var="saleBillInfo"
									...
									<tr>
										<td class="center">
											<input type='checkbox' name='salebackmxId' value="${saleBillInfo.salebackmxId}" /><span class="lbl"></span>
										</td>
			...
			<div id="page" class="pagination">
				<form action="<%=basePath%>salebackAccountController.do?method=entitySalebackcancelaccountbill" name="pageForm" method="post">
...
<script>
	...
	function goHistory(){
		...
		dataCollectForm.action = "<%=basePath%>salebackAccountController.do?method=entitySalebackcancelaccountBatch";
		dataCollectForm.submit();
	}
	//返售撤销记账
	function cancelaccount(){
		 var checkNum = getCheckNum("salebackmxId");
	   	 if (checkNum == 0){
	   		bootbox.alert("请至少选择一条要进行撤销的记录");
	   		return;
	   	 }
		var salebackmxIds = getCheckStr("salebackmxId");//清单主键
		...
		dataCollectForm.action = "<%=basePath%>salebackAccountController.do?method=entitySalebackcancelaccount";
		dataCollectForm.submit();
	}
