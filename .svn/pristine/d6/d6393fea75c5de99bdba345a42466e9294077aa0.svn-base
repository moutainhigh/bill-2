<%-- 
 * 文件名称: out_elec_storage_bill_list.jsp.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
			...
			<%-- 按钮操作区 --%>
				...
				<div>
					<div id="page" class="pagination">
							<form action="<%=basePath%>outStorageController.do?method=elecStorageBillList"  name="pageForm" method="post">
...
<script>
	...
	function goHistory(){
        ...
        dataCollectForm.action="<%=basePath%>outStorageController.do?method=elecStorageBatchList";
		...
		var checkNum = getCheckNum("ids");
	   	...
	   	bootbox.confirm("确定要出库选中的记录吗?", function(result) {
			if(result) {
				...
				$.ajax({
					type: "POST",
					url: '<%=basePath%>outStorageController.do?method=elecStorage',
