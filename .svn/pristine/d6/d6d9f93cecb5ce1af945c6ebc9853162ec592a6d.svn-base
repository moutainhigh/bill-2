<%-- 
 * 文件名称: disc_elec_review_bill_list.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
...
<body>
	<div class="clearfix">
		<div id="page-content" class="page-content">
				...
				<%-- <input type="hidden" name="acctNo" value="${query.acctNo}"/>  --%>
					...
					<div class="row-fluid">
						...
						<span class="temp buyPayRate">
						    ...
						    <input type="text" class="input-medium buyPayRate" name="buyPayRate" disabled value="${fns:formateRate(batch.buyPayRate)}" />
						</span>
			...
			<%-- 按钮操作区 --%>
			<div id="center">
				<form id="btnForm" name="btnForm" action="<%=basePath%>discAccountController.do?method=elecAccount" method="post">
					<div class="row-fluid">
						...
						<div class="span6" id="btn-right">		
							<a class="btn-mini pull-right" href="javascript:reviewSubmit();">提交</a>
			...
			<div id="footer" class="footer">
				...
				<form  action="#" name="dataCollectForm" method="post"></form>
					<div id="page" class="pagination">
						<form action="<%=basePath%>discAccountController.do?method=elecReviewList"  name="pageForm" method="post">
...
function reviewSubmit(){ 
		...
		var bill_ids = getCheckStr("ids");
	   	...
	   	bootbox.confirm("确定要提交选中的记录吗?", function(result) {
	   		if(result) {
		dynamicHiddenElement('dataCollectForm','discmxIds',bill_ids);
		...
		dynamicHiddenElement('dataCollectForm','custAccount',custAccount);
		modal("#layer_loading,#image");//点击票据管理按钮，弹出‘加载中’的遮罩层
		dataCollectForm.action = "<%=basePath%>discAccountController.do?method=doElecAccount";
	...
	function goHistory(){
		...
		btnForm.action = "<%=basePath%>discAccountController.do?method=elecAccount";
		btnForm.submit();
	}
	//详情页面
