...
<div id="page-content" class="page-content">
	...
	<div id="center" class="f-filter">
		<form action="<%=basePath%>receiveMoneyController.do?method=revokeBillList"  method="post" id="" name="custForm" style="margin-bottom:0px;" class="form-search">
			<div class="row-fluid">
				<div class="span6"  id="btn-left">
				</div>
				<div class="span6" id="btn-right">
				  	<a class="btn-mini pull-right" onclick="goBack();">返回</a>
					<a class="btn-mini pull-right" onclick="submitacct();">提交撤销</a>
				</div>
			</div>
		</form>
	</div>
...
<script>
	...
	function goBack(){
		modal("#layer_loading,#image");//点击票据管理按钮，弹出‘加载中’的遮罩层
		dataCollectForm.action = "<%=basePath%>receiveMoneyController.do?method=revokeBillList";
		dataCollectForm.submit();
	}
	//详情页面
