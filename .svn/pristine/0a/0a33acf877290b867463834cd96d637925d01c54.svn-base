			...
			<div id="header">
				<form action="<%=basePath%>saleApplyController.do?method=toAddBill" id="page-content" method="post" name="searchForm" id="searchForm" class="form-search">
					...
					<div class="row-fluid">
						...
						<input type="text" id="billNo" name="billNo" class="input-medium" value="${query.billNo}" />
						<label class="no-padding-right" for="billMoney">票面金额</label>
						<input class="date-medium" type="text" id="startMoney" name="startMoney" value="${query.startMoney}" /> 
						<span class="wave-line">~</span>
						<input class="date-medium" type="text" id="endMoney" name="endMoney" value="${query.endMoney}"/>
						<label class="no-padding-right" for="dueDt">票面到期日</label>
						<input class="date-medium input-date"  name="startDay" id="startDay"  value ="${query.startDay}" valid="required" type="text">
						<span class="wave-line">~</span>
						<input class="date-medium input-date"  name="endDay"id="endDay"  value ="${query.endDay}" valid="required" type="text">
						...
						<c:choose>
							<c:when test="${not empty resultList}">
								<c:forEach items="${resultList}" var="billInfo"
									...
									<tr>
										<td class="center">
											<input type="checkbox" name="ids" value="${billInfo.id}" />
										</td>
...
<script type="text/javascript">
	//详情页面
	function goBillInfo(rgctId){
	   	 var diag = new top.Dialog();
		 diag.Drag = true;
		 diag.Title ="查看";
		 diag.URL = '<%=basePath%>saleApplyController.do?method=goRgctBillByRgctId&rgctId='+rgctId;
		 diag.Width = 920;
		 diag.Height = 550;
