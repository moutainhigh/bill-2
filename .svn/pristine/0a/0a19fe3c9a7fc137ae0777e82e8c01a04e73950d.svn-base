<%-- 
 * 文件名称:buyback_account_bill_list.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
		...
		<div id="page-content" class="page-content">
			...
			<div>
				<div id="page" class="pagination">
					<form action="<%=basePath%>buybackAccountController.do?method=accountBillList" method="post" name="pageForm">
...
<script>
	...
	function account(){
		...
		Form.action = "<%=basePath%>buybackAccountController.do?method=entityAccount";
		Form.submit();
	}
	//返回
	function goHistory(){
		modal("#layer_loading,#image");//点击票据管理按钮，弹出‘加载中’的遮罩层
		Form.action = "<%=basePath%>buybackAccountController.do?method=entityAccountBatchList";
		Form.submit();
	}
	//详情页面
