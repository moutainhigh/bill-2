 ...
 * 可质押的票据查询
 */
public class T107001Service extends OnlineBaseService {
	...
	protected void action(Context context) throws Exception {
		Var107001 transVar = new Var107001();
	 ...
	 */
	protected void CheckData(Context context, Var107001 var107001) throws BizAppException{
		String request = (String)ContextUtil.getRequestData(context);
		Map<String, Class> clazzMap = new HashMap<String, Class>();
		clazzMap.put("Document", Var107001.class);
		Var107001 temp=(Var107001)XmlBeanUtil.xml2Bean(clazzMap, request);
		if (StringUtils.isEmpty(temp.getCustAccount())){
			throw new BizAppException(IErrorNo.ERR_CUSTACCOUNTNULL, "客户账号未上送");
		}
		BeanUtils.copyProperties(temp, var107001);
	}
	 ...
	 * @throws BizAppException
	 */
    protected void ToLocal(Context context, Var107001 var107001) throws BizAppException{
    	RcBaseSearchBean searchBean=new RcBaseSearchBean();
        searchBean.setHoldAcctNo(var107001.getCustAccount());
        searchBean.setBillClass(IDict.K_BILL_CLASS.K_ELEC_BILL);
        List<Var107001InfoBean> result =new ArrayList<Var107001InfoBean>();
   ...
   //     String sql="select billinfo.*,billhist.* from trgct_bill_info billinfo, trgct_bill_hist billhist where billinfo.hist_id=billhist.hist_id";
		...
		try {
			rgctBill = session.getBeanListByListForPage(
					sql,
					RgctBill.class,
					Integer.valueOf(var107001.getCurrentPage()),
					Integer.valueOf(var107001.getPageSize()),
					list);
			var107001.setTotalRows(Integer.toString(session.account("select count(*) from ("+sql+")")));
		...
		if(rgctBill.size() <= 0){
    		...
    		for (RgctBill rgctBillList:rgctBill) {
    			Var107001InfoBean bean=new Var107001InfoBean();
     ...
     * @throws BizAppException
     */
    protected void PackAnswer(Context context, Var107001 var107001) throws BizAppException{
        ...
        List<Var107001InfoBean> infoList=var107001.getResult();
        Var107001Result result=new Var107001Result();
        result.setProResult(proResult);
        result.setResult(infoList);
        
        Map<String, Class> clazzMap = new HashMap<String, Class>();
        clazzMap.put("Document", Var107001Result.class);
        clazzMap.put("info", Var107001InfoBean.class);
