<%-- 
 * 文件名称: disc_audit_batch_list.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
...
<body>
	<div class="clearfix">
		<div id="page-content" class="page-content">
			<div id="header">
				<form action="<%=basePath%>discAuditController.do?method=auditList" method="post" id="page-content" name="searchForm" class="form-search">
				<%-- 查询区  --%>
					<div class="row-fluid">
						<label class="no-padding-right" for="batchNo">批次号</label>
						<input type="text" id="batchNo" name="batchNo" class="input-medium" valid="required" value="${batchNo}" placeholder="请输入批次号"/>
						<a class="btn-mini" id="search" type="button" onclick="doSearch();">查询</a>
					</div>		
				</form>
			</div>
			<%-- 按钮操作区 --%>
			<div id="center">
				<form  id="btnForm">
					<div class="row-fluid">
						<div class="span6" id="btn-left" >
							<a class="btn-mini" href="javascript:auditDetailList();">下一步</a>
					   </div>
		     ...
		     <div id="footer">
				 <table class="table table-striped table-bordered table-hover" id="tab" cellpadding="0" cellspacing="0">
					 <thead>
						 <tr>
						    <th class="center"><input type="checkbox" name="allcheck" disabled="disabled" id="allcheck" onclick="selectAll('allcheck','discId')"/></th>
			...
			<div id="page" class="pagination">
				<form action="<%=basePath%>discAuditController.do?method=auditList"  name="pageForm" method="post">
...
<script>
		...
		function auditDetailList(){
			...
			location.href="<%=basePath%>discAuditController.do?method=auditDetailList&discId="+discId;
		}
		//详情页面
