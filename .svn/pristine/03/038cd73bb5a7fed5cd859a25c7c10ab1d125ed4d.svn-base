<%-- 
 * 文件名称:disc_cancel_apply_detail_list.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
		...
		<div id="page-content" class="page-content">
			...
			<div>
				<div id="page" class="pagination">
					<form action="<%=basePath%>discApplyController.do?method=cancelApplyDetailList" method="post" name="pageForm">
...
<script>
	...
	function cancelApply(){
		...
		$.ajax({
			url:"<%=basePath%>discApplyController.do?method=cancelApply",
			type:"POST",
			data:{"ids":ids},
			dataType:"json",
			success:function(rs){
				if(rs.success){
					...
					Form.action = "<%=basePath%>discApplyController.do?method=cancelApplyList";
					Form.submit();
				}else{
					bootbox.alert("撤销失败");
				}
	...
	function goHistory(){
		modal("#layer_loading,#image");//点击票据管理按钮，弹出‘加载中’的遮罩层
		dataCollectForm.action = "<%=basePath%>discApplyController.do?method=cancelApplyList";
