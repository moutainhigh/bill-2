<%-- 
 * 文件名称: disc_storage_detail_list.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
...
<body>
	<div class="clearfix">
		...
		<div id="page" class="pagination">
			<form action="<%=basePath%>discStorageController.do?method=storageDetailList" method="post" name="pageForm">
		...
		var checkNum = getCheckNum("ids");
	   	...
	   	bootbox.confirm("确定要入库选中的记录吗?", function(result) {
			if(result) {
				...
				$.ajax({
					type: "POST",
					url: '<%=basePath%>discStorageController.do?method=doStorage',
			    	data: {'discmx_ids': discmx_ids},
					dataType:'json',
					...
					success: function(data){	
						if (data.success){  //入库成功
							...
							publicForm.action="<%=basePath%>discStorageController.do?method=storageBatchList";
							publicForm.submit();
						} else {
							top.hangge();
							bootbox.alert("入库失败!"); 
