<%--
 * 文件名称: saleback_confirm_bill_list_elec.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 票据列表页面
			...
			<div id="header">
				<form action=""	method="post" name="infoForm" id="infoForm" class="form-search">
					...
					<div class="row-fluid threeLine">
						...
						<input class="input-medium input-date layinput" name="createDt"  id="createDt" value="${batch.createDt}" type="text" class="input-medium" placeholder="系统日期" title="请输入有效的日期 (YYYY-MM-DD)" valid="required dateISO" readonly/>
					</div>
				...
				<form  id="btnForm">
					<div class="row-fluid">
						...
						<div class="span6" id="btn-right">
							<a class="btn-mini pull-right" onclick="submitacct();">提交</a>
							...
							<c:when test="${not empty resultList}">
								<c:forEach items="${resultList}" var="saleBillInfo"
									...
									<tr>
										...
										<td class="center">
										    <c:if test="${saleBillInfo.operStatus=='BSb604'}">背书成功状态</c:if>
											<c:if test="${saleBillInfo.operStatus=='BSb605'}">交易对手方已签收</c:if>
											<c:if test="${saleBillInfo.operStatus=='BSb606'}">交易对手方已驳回</c:if>
										</td>
										<td class="center"><a
											href="javascript:goDetail('${saleBillInfo.rgctId}')">查看</a>
										</td>
									</tr>
								</c:forEach>
							</c:when>
			...
			<form  action="#" name="dataCollectForm" method="post"></form>
			 <%--/列表操作区--%>
				<div id="page" class="pagination">
					<form action="<%=basePath%>salebackAccountController.do?method=searchSaleBackConfirmBillList"  name="pageForm" method="post">
						<%@ include file="/webpage/system/admin/pageFormContent.jsp"%> 
						<input type="hidden" name="salebackId" id="salebackId" value="${batch.salebackId}" />
					</form>
				</div>
			</div>
		</div>
	...
	function submitacct(){
		...
		bootbox.confirm("确定要提交选中的记录吗?", function(result) {
			if(result){
					...
					$.ajax({
						type: "POST",
						url: '<%=basePath%>salebackAccountController.do?method=doConfirmSubmit',
						...
						success: function(data){	
							if (data.success){  //处理成功
			                    ...
			                    dataCollectForm.action = "<%=basePath%>salebackAccountController.do?method=searchConfirmableSalebackBatch";
