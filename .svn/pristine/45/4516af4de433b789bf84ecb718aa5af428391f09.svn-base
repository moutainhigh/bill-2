...
<body style="font-family:'微软雅黑';" id="indexBody">
	<div id="home">
		<!--页面顶部 -->
		<div class="navbar navbar-default" id="navbar" style="width:100%;">
			<div id="navbar-container">
				...
				<div class="navbar-header pull-right" role="navigation" id="navbarHeader">
					<ul class="nav ace-nav">
						...
						<li class="" title="修改密码">
							<a  onclick="modifyPwd('${user.userId}');" class="bgColor" style="cursor:pointer" >
								修改密码
							</a>
						</li>
						<li class="" title="退出">
	    ...
	    <div class="main-container ace-save-state" id="main-container" style="width:100%;">
			...
			<div class="sidebar responsive ace-save-state" id="sidebar">
				...
				<!-- <ul class="nav nav-list" id="subMenus"></ul> -->
			</div>
			<!--/左侧菜单-->
		...
		background-size:cover;">
						<iframe name="mainFrame" id="mainFrame" frameborder="0" src="loginController.do?method=tab" style="" ></iframe>
					</div>
				</div>
			</div>
		</div>
...
<script>
	$(document).ready(function(){
		localStorage.setItem("indexId","home");
		wSizeWidth();
		function wSizeWidth(){
		...
		function gotoMenuTab(menuCode,name,ourl){
			...
			if(tabText.length==0){
				...
				if(flag==true){
					...
					$("#mainFrame").contents().find("#jerichotab_contentholder div").eq(k).next().css('height','inherit');
				}else{
					oSearchNotice.style.display="none";
					var url=ourl;
