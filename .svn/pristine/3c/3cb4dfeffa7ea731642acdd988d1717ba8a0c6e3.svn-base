/********************************************
 * 文件名称: IBbspProductService.java
 * 系统名称: 基础技术平台V2.0
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
 * 系统版本: 2.0.0.1
 * 开发人员: dq
 * 开发时间: 2016-7-12 上午11:12:52
 * 审核人员:
 * 相关文档:
 * 修改记录: 修改日期    修改人员    修改说明
 *********************************************/
package com.herongtech.console.service.interfaces;

import com.herongtech.console.domain.bean.BbspProduct;
import com.herongtech.exception.impl.BizAppException;

public interface IBbspProductService {
	
	public static final String HUNDSUN_VERSION="@system 票据管理平台 @version 2.0.0.1 @lastModiDate @describe ";
	/**
	 * 获取产品维护表
	 * 
	 * @param id ID 
	 */
	public BbspProduct getBbspProduct(String prodNo) throws BizAppException;
	
	/**
	 * 插入产品维护表
	 *
	 */
	public void addBbspProduct(BbspProduct bbspProduct) throws BizAppException;
	
	/**
	 * 修改产品维护表
	 */
	public void modifyBbspProduct(BbspProduct bbspProduct) throws BizAppException; 
}
