<%--
 * 文件名称: saleback_revoke_endorse_bill_list_elec.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 批次下清单列表
	...
	<div class="clearfix">
		<div id="page-content" class="page-content">
				...
				<form id="btnForm">
					<div class="row-fluid">
						...
						<div class="span6" id="btn-right">
							<a class="btn-mini pull-right" onclick="apply();">撤销背书</a>
							...
							<c:when test="${not empty resultList}">
								<c:forEach items="${resultList}" var="saleBillInfo"
									...
									<tr>
										...
										<td class="center">
	                                      <a href="javascript:goDetail('${billList.rgctId}')">查看</a>	</td>
									</tr>
								</c:forEach>
							</c:when>
							<c:otherwise>
			...
			<div id="page" class="pagination">
				<form action="<%=basePath%>SalebackApplyController.do?method=searchsalebackcxendorselistElec" name="pageForm" method="post">
...
<script>
	...
	function goHistory(){
		...
		dataCollectForm.action = "<%=basePath%>SalebackApplyController.do?method=searchEndorseRevokableSalebackBatch";
	...
	function apply(){
		...
		dynamicHiddenElement('dataCollectForm','salebackmxIds',salebackmxId);
	  	dynamicHiddenElement('dataCollectForm','salebackId',salebcakId);
		dataCollectForm.action = "<%=basePath%>SalebackApplyController.do?method=revokeSalebackEndorse";
		dataCollectForm.submit();
	}
	//详情页面
