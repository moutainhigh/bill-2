<%-- 
 * 文件名称: disc_cancel_apply_list.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
...
<body>
	<div class="clearfix">
		<div id="page-content" class="page-content">
		   	<div id="header">
				<form action="<%=basePath%>discApplyController.do?method=cancelApplyList"  method="post" id="page-content" name="Form" style="padding:0px;" class="form-search">
			    <%-- 查询区  --%>
					<div class="row-fluid">
		    ...
		    <div id="footer">
				<table class="table table-striped table-bordered table-hover" id="tab" cellpadding="0" cellspacing="0">
					<thead>
						<tr>
							<th class="center"><input type="checkbox" disabled="disabled"/>
			...
			<div>
				<div id="page" class="pagination">
					<form action="<%=basePath%>discApplyController.do?method=cancelApplyList" method="post" name="pageForm">
...
<script>
	...
	$("#pageContent a").click(function(){
		...
		This.siblings('a').addClass('btn-white');
	})
	//撤销申请
	function cancelApply(){
		...
		Form.action = "<%=basePath%>discApplyController.do?method=cancelApplyDetailList";
		Form.submit();
	}
	//详情页面
