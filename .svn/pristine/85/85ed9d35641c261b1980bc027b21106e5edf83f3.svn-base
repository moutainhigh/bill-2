			...
			<%-- 按钮操作区 --%>
					...
					<form  id="btnForm">
						<div class="row-fluid">
						   ...
						   <div class="span6" id="btn-right">
								<a class="btn-mini pull-right" href="javascript:doStorage();">出库</a>
						...
						<tbody>
							<c:choose>
								<c:when test="${not empty resultList}">
									<c:forEach items="${resultList}" var="get" varStatus="vs">
										<tr>
											<td class="center"><input type="checkbox" name="ids" value="${get.getmxId}"/></td>
											<td class="center">${get.billNo}</td>
											<td class="center">${fns:getDictLabel('K_BILL_TYPE',get.billType)}</td>
											<td class="center">${get.issueDt}</td>
											<td class="center">${get.dueDt}</td>
											<td class="center">${fns:formateMoney(get.billMoney)}</td>
											<td class="center">${get.acceptor}</td>
											<td class="center">${get.remitter}</td>
											<td class="center"><a href="javascript:goBillDetail('${get.getmxId}')">查看</a></td>
										</tr>
									</c:forEach>
								</c:when>
								<c:otherwise>
				...
				<div>
					<div id="page" class="pagination">
							<form action="<%=basePath%>repurCollateStorageController.do?method=storageBillList"  name="pageForm" method="post">
							<%@ include file="/webpage/system/admin/pageFormContent.jsp"%>
							<input type="hidden" name="getId" value="${batch.getId}"/>
						</form>
					</div>
				</div>
			</div>
...
<script>
	...
	function goHistory(){
        ...
        dataCollectForm.action="<%=basePath%>repurCollateStorageController.do?method=storageBatchList";
	...
	function doStorage(){
		var checkNum = getCheckNum("ids");
	   	if (checkNum <= 0){
	   		bootbox.alert("请先选择要出库的记录!");
	   		return;
	   	}
	   	var getmx_ids = getCheckStr("ids");
	   	var getId = "${getId}";
	   	bootbox.confirm("确定要出库选中的记录吗?", function(result) {
			if(result) {
				top.jzts();
				modal("#layer_loading,#image");//点击票据管理按钮，弹出‘加载中’的遮罩层
				$.ajax({
					type: "POST",
					url: '<%=basePath%>repurCollateStorageController.do?method=storage',
			    	data: {'getmx_ids': getmx_ids,'getId': getId},
					dataType:'json',
					cache: false,
	...
	//详情页面
	function goBillDetail(getmxId){
	   	 var diag = new top.Dialog();
		 diag.Drag = true;
		 diag.Title ="详情";
		 diag.URL = "<%=basePath%>repurCollateApplyController.do?method=goBillDetail&getmxId="+getmxId;
		 diag.Width = 990;
		 diag.Height = 260;
