	...
	<div class="clearfix">
		<div id="page-content" class="page-content">
			...
			<div id="page" class="pagination">
				<form action="<%=basePath%>saleApplyController.do?method=elecBatch"
...
<script>
	...
	function add(){
		...
		diag.Drag = true;
	 	diag.Title ="新增";
	 	diag.URL = "<%=basePath%>saleApplyController.do?method=toAddBatch&billClass=2";
	 	diag.Width = 800;
	 	diag.Height =600;
	...
	function edit(saleId){
		 ...
		 diag.Drag = true;
		 diag.Title ="修改";
		 diag.URL = "<%=basePath%>saleApplyController.do?method=toEditBatch&billClass=2&saleId="+saleId;
		 diag.Width = 800;
		 diag.Height = 600;
		 diag.CancelEvent = function(){ //关闭事件
		 	dataCollectForm.action = "<%=basePath%>saleApplyController.do?method=elecBatch";
	...
	function billManage(){
		var checkNum = getCheckNum("saleId");
	   	 if (checkNum !=1){
	   		bootbox.alert("请先选择要票据管理的记录");
	   		return;
	   	 }
	   	 var saleId = getCheckStr("saleId");
	   	dynamicHiddenElement('dataCollectForm','saleId',saleId);
	   	dynamicHiddenElement('dataCollectForm','billClass','2');
	...
	function goApplyInfo(saleId){
		 ...
		 diag.Drag = true;
		 diag.Title ="详情";
		 diag.URL = '<%=basePath%>saleApplyController.do?method=goApplyInfo&saleId='+saleId+'&flag=elecApply';
		 diag.Width = 990;
		 diag.Height = 510;
		 diag.CancelEvent = function(){ //关闭事件
			diag.close();
