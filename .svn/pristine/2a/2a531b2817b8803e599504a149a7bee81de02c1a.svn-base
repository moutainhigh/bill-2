		...
		<div id="page-content" class="page-content">
			...
			<div>
				<div id="page" class="pagination">
						<form action="<%=basePath%>intoAuditController.do?method=elecAuditBillList"  name="pageForm" method="post">
...
<script>
	...
	function goHistory(){
        ...
        dataCollectForm.action="<%=basePath%>intoAuditController.do?method=elecAuditBatchList";
        dataCollectForm.submit();
	}
	function audit(){
		 var checkNum = getCheckNum("ids");
	   	 if (checkNum == 0){
	   		bootbox.alert("请选择记录审核");
	   		return;
	   	 }
		...
		var intoId = '${batch.intoId}';
		 ...
		 diag.Drag = true;
		 diag.Title ="审核";
		 diag.URL = "<%=basePath%>intoAuditController.do?method=toElecAudit&ids="+ids+"&intoId="+intoId;
		 diag.Width = 430;
		 diag.Height =230;
		 diag.CancelEvent = function(){ //关闭事件
	        ...
	        dataCollectForm.action = "<%=basePath%>intoAuditController.do?method=elecAuditBatchList"
	        dataCollectForm.submit();
		//	location.href ="<%=basePath%>intoAuditController.do?method=auditBatchList";
		 };
		 diag.show();
	}
	//详情页面
