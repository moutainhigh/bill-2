								...
								<c:when test="${not empty batchList}">
									<c:forEach items="${batchList}" var="saleApplyInfo" varStatus="vs">
										<tr>
											...
											<td class="center">${fns:getDictLabel('K_YORN',saleApplyInfo.forbidFlag)}</td>
											<td class="center">
				...
				<form  action="#" name="dataCollectForm" method="post"></form>
				 ...
				 <div>
					<div id="page" class="pagination">
						<form action="<%=basePath%>saleAccountController.do?method=queryApplyForSaleAcct"  name="pageForm" method="post">
...
<script>
	function billManage(){
		...
		dataCollectForm.action = "<%=basePath%>saleAccountController.do?method=toSubmitSaleAccount";
	...
	function goApplyInfo(saleId){
		 ...
		 diag.Drag = true;
		 diag.Title ="详情";
		 diag.URL = '<%=basePath%>saleApplyController.do?method=goApplyInfo&saleId='+saleId+'&flag=account';
		 diag.Width = 1000;
		 diag.Height = 505;
