 ...
 * 修改记录: 修改日期    修改人员    修改说明
 *********************************************/
package com.herongtech.online.trans.trans102002;
 ...
 * 查询可执行撤销提示承兑服务
 */
public class T102002Service extends OnlineBaseService {
	...
	protected void action(Context context) throws Exception {
		Var102002 transVar = new Var102002();
	 ...
	 */
	protected void CheckData(Context context, Var102002 var102002) throws BizAppException{
		String request = (String)ContextUtil.getRequestData(context);
		
		Map<String,Class> clazzMap = new HashMap<String,Class>();
		clazzMap.put("Document", Var102002.class);
		Var102002 temp = (Var102002)XmlBeanUtil.xml2Bean(clazzMap, request);
		
//		TODO BusiDate busiDate = (BusiDate) ContextUtil.getContextAttribute(context, IConstants.SYSBUSININFO);
		
		if (StringUtils.isEmpty(temp.getCustAccount())){
			throw new BizAppException(IErrorNo.ERR_CUSTACCOUNTNULL, "客户账号未上送");
		}
		
		var102002.setCustAccount(temp.getCustAccount());
		var102002.setCurrentPage(temp.getCurrentPage());
        var102002.setPageSize(temp.getPageSize());
	}
	 ...
	 * @throws BizAppException
	 */
    protected void ToLocal(Context context, Var102002 var102002) throws BizAppException{
    	RcBaseSearchBean searchBean = new RcBaseSearchBean();
    	searchBean.setHoldAcctNo(var102002.getCustAccount());
    	searchBean.setBillClass(IDict.K_BILL_CLASS.K_ELEC_BILL);
    	
    	IDB session = DBFactory.getDB();
    	String sql =RcServiceFactory.getRcAcptBillService().queryReversibleBill(searchBean);
    	List<RgctBill> rgctBillList = null;
		try {
			List<Object> list = new ArrayList<Object>();
			rgctBillList = session.getBeanListByListForPage(sql, RgctBill.class, Integer.parseInt(var102002.getCurrentPage()), Integer.parseInt(var102002.getPageSize()), list);
			var102002.setTotalRows(Integer.toString(session.account("select count(*) from ("+sql+")")));
    	...
    	var102002.setBillList(this.getVar102002InfiBeanByRgctBillList(rgctBillList));
    	
    	/*try {
			Var102002.setTotRecNum(
     ...
     * @throws BizAppException
     */
    protected void PackAnswer(Context context, Var102002 var102002) throws BizAppException{
    	TransInfo trans=(TransInfo)ContextUtil.getContextAttribute(context, IConstants.TRANS_INFO);
    	Var102002Result resp = new Var102002Result();
    	resp.setType("S");
        resp.setExSerial(trans.getExSerial());
        resp.setFunctionId(trans.getFunctionId());
    	resp.setBillList(var102002.getBillList());
    	resp.setCurrentPage(var102002.getCurrentPage());
    	resp.setRetNum(Integer.toString(var102002.getBillList().size()));
    	resp.setTotalRows(var102002.getTotalRows());
    	
    	Map<String,Class> clazzMap = new HashMap<String,Class>();
    	clazzMap.put("Document",Var102002Result.class);
    	clazzMap.put("bean", Var102002InfoBean.class);
	...
	public List<Var102002InfoBean> getVar102002InfiBeanByRgctBillList(List<RgctBill> rgctBillList){
		List<Var102002InfoBean> billList = new ArrayList<Var102002InfoBean>();
		for(RgctBill rgctBill : rgctBillList){
			Var102002InfoBean bean = new Var102002InfoBean();
