<%--
 * 文件名称: saleback_entity_cancel_account_batch.jsp
 * 系统名称: 票据管理平台
 * 模块名称: 纸票返售撤销记账批次页面
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
			...
			<div id="center">
				<div class="row-fluid">
					<div class="span6" id="btn-left" >
						<a class="btn-mini" href="javascript:nextStep();">票据管理</a>
					</div>
					 ...
					 <c:choose>
							<c:when test="${not empty batchList}">
								<c:forEach items="${batchList}" var="batchList" varStatus="vs">
									<tr>
										<td class="center">
											<input type='checkbox' name='rebuyId' value="${batchList.salebackId}" /><span class="lbl"></span>
										</td>
										...
										<td class="center">${batchList.buyDt}</td>
										<td class="center">${batchList.salebackOpenDt}</td>
										<td class="center">${batchList.salebackDueDt}</td>
									</tr>
								</c:forEach>
							</c:when>
							<c:otherwise>
			...
			<form  action="#" name="dataCollectForm" method="post"></form>
			 ...
			 <div>
				<div id="page" class="pagination">
					<form action="<%=basePath%>salebackAccountController.do?method=entitySalebackcancelaccountBatch"  name="pageForm" method="post">
...
<script>
	...
	var checkNum = getCheckNum("rebuyId");
		...
		if (checkNum !=1){
	  		bootbox.alert("一次只能对一个批次进行撤销申请");
	  		return;
	  	}
	  	modal("#layer_loading,#image");
	  	var salebackId = getCheckStr("rebuyId");	
	  	dynamicHiddenElement('dataCollectForm','salebackId',salebackId);
		dataCollectForm.action = "<%=basePath%>salebackAccountController.do?method=entitySalebackcancelaccountbill";
