...
<div class="clearfix">
	<div id="page-content" class="page-content">
		<div id="header">
			<form action="<%=basePath%>collateralizationApplyController.do?method=searchElecBill"  method="post" name="custForm" id="custForm" class="form-search">
				<%-- 查询区  --%>
				<div class="row-fluid">
					...
					<label class="col-md-2 control-label no-padding-right" for="custName"><span class="star">*</span>客户名称</label>
					...
					<c:choose>
							<c:when test="${not empty resultList}">
								<c:forEach items="${resultList}" var="save" varStatus="vs">
									<tr>
										<td class="center">
											<input type='checkbox' name='ids' value="${save.savemxId}" onclick="getall('ids')" price="${save.billMoney}"/><span class="lbl"></span>
										</td>
...
<script>
	...
	function toCheck(){
		...
		bootbox.confirm("确定要接收选中的记录吗?", function(result) {
			if(result){
				...
				$.ajax({
					type: "POST",
					url: '<%=basePath%>collateralizationApplyController.do?method=elecCheckBill',
			    	data: {'ids': ids},
					dataType:'json',
	...
	function toSign(ids,billType){
		 ...
		 dataCollectForm.action = "<%=basePath%>collateralizationApplyController.do?method=elecSignBill";
	...
	function refuse(){
		 ...
		 diag.Drag = true;
		 diag.Title ="拒收意见";
		 diag.URL = "<%=basePath%>collateralizationApplyController.do?method=elecRefuseRemark&ids="+ids;
		 diag.Width = 430;
		 diag.Height =200;
		 diag.CancelEvent = function(){ //关闭事件
		 diag.close();
		 dataCollectForm.action = "<%=basePath%>collateralizationApplyController.do?method=searchElecBill&custNo="+custNo;
         dataCollectForm.submit();
		 };
		 diag.show();
	}
