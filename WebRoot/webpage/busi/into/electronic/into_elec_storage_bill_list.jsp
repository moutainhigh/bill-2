	...
	<div class="clearfix">
		...
		<div>
			<div id="page" class="pagination">
					<form action="<%=basePath%>intoStorageController.do?method=elecStorageBillList"  name="pageForm" method="post">
...
<script>
	...
	function goHistory(){
        ...
        dataCollectForm.action="<%=basePath%>intoStorageController.do?method=elecStorageBatchList";
	...
	function doStorage(){
		var checkNum = getCheckNum("ids");
	   	if (checkNum <= 0){
	   		bootbox.alert("请选择需要入库的记录");
	   		return;
	   	}
	   	var intomx_ids = getCheckStr("ids");
	   	var intoId = "${intoId}";
	   	bootbox.confirm("是否确定要入库吗?", function(result) {
			if(result) {
				top.jzts();
				modal("#layer_loading,#image");
				$.ajax({
					type: "POST",
					url: '<%=basePath%>intoStorageController.do?method=elecStorage',
