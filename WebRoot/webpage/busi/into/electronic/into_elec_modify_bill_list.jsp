...
<body>
	<div class="clearfix">
		<div id="page-content" class="page-content">
			...
			<%-- 按钮操作区 --%>
			<div id="center">
				<form action="<%=basePath%>intoApplyController.do?method=elecModifyApplyBillList"  method="post" id="elecReviewForm" name="elecReviewForm" style="padding:0px;margin-bottom:0px;" class="form-search">
					<div class="row-fluid">
			...
			<form action="<%=basePath%>intoApplyController.do?method=elecModifyApplyBillList"  name="pageForm" method="post" >
					<%@ include file="/webpage/system/admin/pageFormContent.jsp"%>
					<input type="hidden" name="custNo" id="custNo" value="${custNo}"/>
					<input type="hidden" name="intoId" id="intoId" value="${batch.intoId}"/> 
				</form>
			</div>
		</div>
	</div>
...
<script>
	...
	function goHistory(){
	    ...
	    dataCollectForm.action="<%=basePath%>intoApplyController.do?method=elecModifyApplyBatchList";
	...
	function elecApply(){
		var checkNum = getCheckNum("ids");
	   	if (checkNum <= 0){
	   		bootbox.alert("请选择要提交的票据");
	   		return;
	   	}
	   	var ids = getCheckStr("ids");
	   	var billType = "${batch.batchType}";
	   	var intoId = "${batch.intoId}";
	   	var custNo = "${batch.custNo}";
	   	bootbox.confirm("是否确定要提交的票据?", function(result) {
	   		if(result){
	   			top.jzts();
	   			modal("#layer_loading,#image");
				$.ajax({
						url:"<%=basePath%>intoApplyController.do?method=elecModifyApplySubmit",
						type:"POST",
						data:{'intomxIds': ids,'billType': billType,'intoId': intoId,'custNo': custNo},
						dataType:"json",
						cache: false,
	...
	function add(){
	    ...
	    dynamicHiddenElement('dataCollectForm','intoIds',$('#intoId').val());
		dynamicHiddenElement('dataCollectForm','billType',billType);
		modal("#layer_loading,#image");
		dataCollectForm.action = "<%=basePath%>intoApplyController.do?method=elecAddBill";
		dataCollectForm.submit();
	}
	//移除
	function del(){
		var checkNum = getCheckNum("ids");
	   	...
	   	var intoId = "${batch.intoId}";
	   	var custNo = "${batch.custNo}";
		bootbox.confirm("是否确定要移除该电子票据?", function(result) {
			if(result){
					top.jzts();
					modal("#layer_loading,#image");
					$.ajax({
						type: "POST",
						url: '<%=basePath%>intoApplyController.do?method=elecDelBill',
				    	data: {'intomxIds': bill_ids,'billType': billType,'intoId': intoId,'custNo': custNo},
						dataType:'json',
						cache: false,
						success: function(data){	
