...
<div class="clearfix">
	<div id="jump-content" class="page-content">
		<form action="<%=basePath%>outAuditController.do?method=elecAudit" method="post" name="Form" id="Form" class="form-search">
			<div class="row-fluid">
			...
			<div class="row-fluid save">
				<div class="col-xs-12 col-sm-4 col-md-4 center">
					...
					<input type="hidden" name="outId" id="outId" value="${outId}"/>
				</div>
			</div>
		</form>
	</div>
	...
	function save(){
		if($("#Form").valid()){
			if($("#status").val()=="0" && $("#option").val()==""){
				...
				$.ajax({
					url:"<%=basePath%>outAuditController.do?method=elecAudit",
					type:"POST",
					data:$("#Form").serialize(),
					dataType:"json",
					success:function(rs){
