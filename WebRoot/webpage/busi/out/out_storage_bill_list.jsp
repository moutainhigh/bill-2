		...
		<div id="page-content" class="page-content">
							...
							<c:when test="${not empty resultList}">
								<c:forEach items="${resultList}" var="out" varStatus="vs">
									<tr>
										...
										<td class="center"><a href="javascript:goBillDetail('${out.outmxId}')">查看</a></td>
									</tr>
								</c:forEach>
							</c:when>
							<c:otherwise>
			...
			<div>
				<div id="page" class="pagination">
					<form action="<%=basePath%>outStorageController.do?method=storageBillList"  name="pageForm" method="post">
...
<script>
	...
	function goHistory(){
        ...
        dataCollectForm.action="<%=basePath%>outStorageController.do?method=storageBatchList";
		...
		var checkNum = getCheckNum("ids");
	   	...
	   	bootbox.confirm("确定要出库选中的记录吗?", function(result) {
			if(result) {
				...
				$.ajax({
					type: "POST",
					url: '<%=basePath%>outStorageController.do?method=storage',
			    	data: {'outmx_ids': outmx_ids,'outId': outId},
					dataType:'json',
					cache: false,
	...
	//详情页面
	function goBillDetail(outmxId){
	   	 var diag = new top.Dialog();
		 diag.Drag = true;
		 diag.Title ="详情";
		 diag.URL = "<%=basePath%>outApplyController.do?method=goBillDetail&outmxId="+outmxId;
		 diag.Width = 900;
		 diag.Height = 260;
