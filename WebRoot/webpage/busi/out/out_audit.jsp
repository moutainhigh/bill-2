...
<div class="clearfix">
	<div id="jump-content" class="page-content">
		<form action="<%=basePath%>outAuditController.do?method=audit" method="post" name="Form" id="Form" class="form-search">
			<div class="row-fluid">
	...
	function save(){
		if($("#Form").valid()){
			if($("#status").val()=="0" && $("#option").val()==""){
				...
				$.ajax({
					url:"<%=basePath%>outAuditController.do?method=audit",
					type:"POST",
					data:$("#Form").serialize(),
					dataType:"json",
					success:function(rs){
