<%--
 * 文件名称: saleback_endorse_batch_elec.jsp
 * 系统名称: 票据管理平台
 * 模块名称: 电票转入撤销申请批次列表页面
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
			...
			<form  action="#" name="dataCollectForm" method="post"></form>
			 ...
			 <div>
				<div id="page" class="pagination">
					<form action="<%=basePath%>SalebackApplyController.do?method=searchEndorsableSaleBackBatch"  name="pageForm" method="post">
...
<script>
	function nextStep(rebuyId){
		var checkNum = getCheckNum("rebuyId");
		if (checkNum==0){
	  		bootbox.alert("请先选择要背书申请的记录");
	  		return;
	  	}
		if (checkNum !=1){
	  		bootbox.alert("一次只能对一个批次进行背书申请");
	  		return;
	  	}
	  	modal("#layer_loading,#image");
	  	var salebackId = getCheckStr("rebuyId");	
	  	dynamicHiddenElement('dataCollectForm','salebackId',salebackId);
		dataCollectForm.action = "<%=basePath%>SalebackApplyController.do?method=salebackendorseapply";
