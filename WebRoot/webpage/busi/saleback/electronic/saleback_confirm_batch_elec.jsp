<%--
 * 文件名称: saleback_confirm_batch_elec.jsp
 * 系统名称: 票据管理平台
 * 模块名称: 电票转入撤销申请批次列表页面
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
					 ...
					 <c:choose>
							<c:when test="${not empty batchList}">
								<c:forEach items="${batchList}" var="batchList" varStatus="vs">
									<tr>
										<td class="center">
											<input type='checkbox' name='ids' value="${batchList.salebackId}" /><span class="lbl"></span>
										</td>
										...
										<%-- <td class="center"><a onclick="batchDetail('${batchList.salebackId}');">查看</a></td> --%>
									</tr>
								</c:forEach>
							</c:when>
							<c:otherwise>
			...
			<form  action="#" name="dataCollectForm" method="post"></form>
			 ...
			 <div>
				<div id="page" class="pagination">
					<form action="<%=basePath%>salebackAccountController.do?method=searchConfirmableSalebackBatch"  name="pageForm" method="post">
...
<script>
	function nextStep(){
		var checkNum = getCheckNum("ids");
   		if (checkNum !=1){
   			bootbox.alert("一次只能对一个批次进行返售确认");
   			return;
   	 	}
   		var salebackId = getCheckStr("ids");	
	  	modal("#layer_loading,#image");
	  	dynamicHiddenElement('dataCollectForm','salebackId',salebackId);
		dataCollectForm.action = "<%=basePath%>salebackAccountController.do?method=searchSaleBackConfirmBillList";
