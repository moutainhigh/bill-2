	...
	function save(){
		if($("#Form").valid()){
			if($("#status").val()=="0" && $("#reason").val()==""){
				...
				$.ajax({
               	 	type: "POST",
                	url:"<%=basePath%>discAuditController.do?method=auditElec",
                	data:$('#Form').serialize(),
                	async: false,
                	dataType:'json',
                	success: function(data) {
                    	if(data.success){
                    		//bootbox.alert("审核成功");
                    		$("#mainFrame",parent.document).contents().find("#jerichotabiframe_lm020204").contents().find("#audit-result").val("1");
