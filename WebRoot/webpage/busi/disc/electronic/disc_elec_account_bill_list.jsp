<%-- 
 * 文件名称: disc_elec_account_bill_list.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
...
<body>
	<div class="clearfix">
		<div id="page-content" class="page-content">
			<div id="header">
				...
				<%-- <input type="hidden" name="acctNo" value="${query.acctNo}"/>  --%>
					...
					<div class="row-fluid">
						...
						<t:dictSelect  name="payType" other="" className="select-medium" dictGroup="K_PAY_TYPE" defaultVal="${batch.payType}" haveHead="true" valid="required" disabled="disabled">
				        </t:dictSelect>
			...
			<%-- 按钮操作区 --%>
			<div id="center">
				<form id="btnForm" name="btnForm" action="<%=basePath%>discAccountController.do?method=finalAccount" method="post">
					<div class="row-fluid">
			...
			<div id="page" class="pagination">
				<form action="<%=basePath%>discAccountController.do?method=elecAccountBillList"  name="pageForm" method="post">
...
if(buyPayRate=='3'){
    $(".buyPayRate").show();
    $(".buyPayRate label").css("margin-left","25px");
}else{
    $(".buyPayRate").hide();
    $(".buyPayRate + label").children("span").css("margin","29.5");
}
	...
	function finalAccount(){
		...
		if(result){
				...
				$.ajax({
					type: "POST",
					url: '<%=basePath%>discAccountController.do?method=doFinalAccount',
					...
					success: function(data){	
						if (data.success){  //处理成功
						   if(data.attributes.count == "0"){
						    ...
						    dataCollectForm.action = "<%=basePath%>discAccountController.do?method=finalAccount";
		                    dataCollectForm.submit();
						   }else{
							dynamicHiddenElement('dataCollectForm','acctNo',custAccount);
		                    dynamicHiddenElement('dataCollectForm','discId',discId);
		                    modal("#layer_loading,#image");
		                    dataCollectForm.action = "<%=basePath%>discAccountController.do?method=elecAccountBillList";
		                    dataCollectForm.submit();
						   }
						} else {
	...
	function goHistory(){
		modal("#layer_loading,#image");
		btnForm.action = "<%=basePath%>discAccountController.do?method=finalAccount";
		btnForm.submit();
	}
