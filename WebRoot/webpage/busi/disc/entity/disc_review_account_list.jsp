<%-- 
 * 文件名称: disc_review_account_list.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
...
<body>
	<div class="clearfix">
		<div id="page-content" class="page-content">
			<div id="header">
				<form action="<%=basePath%>discAccountController.do?method=reviewAccountList"  method="post" id="Form" name="Form" style="padding:0px;" class="form-search">
			    <%-- 查询区  --%>
					<div class="row-fluid">
						<label class="no-padding-right" for="batchNo">批次号</label>
						<input type="text" class="input-medium" id="batchNo" name="batchNo" class="input-medium" valid="required" value="" placeholder="请输入批次号"/>
    		...
    		<div id="footer">
				<table class="table table-striped table-bordered table-hover" id="tab" cellpadding="0" cellspacing="0">
					<thead>
						<tr>
							<th class="center"><input type="checkbox" id="selectall" disabled="disabled" /></th>
					...
					<c:choose>
							<c:when test="${not empty batchList}">
								<c:forEach items="${batchList}" var="disc" varStatus="vs">
									<tr>
										<td class="center"><input type="checkbox" onclick="radioStyle(this,'discId')" name="discId" value="${disc.discId}"/><span class="lbl"></span>
										</td>
			...
			<div id="page" class="pagination">
				<form action="<%=basePath%>discAccountController.do?method=reviewAccountList" method="post" name="pageForm">
...
<script>
	...
	function reviewList(){
		...
		dynamicHiddenElement('Form','discId',discId);
		Form.action="<%=basePath%>discAccountController.do?method=reviewDetailList";
		Form.submit();
	}
	//详情页面
