<%-- 
 * 文件名称: revoke_rebuy_audit_batch_elec.jsp
 * 系统名称: 票据管理平台
 * 模块名称: 电票转入撤销审核批次页面
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
...
<script>
	function searchRevokableDetail(rebuyId){
		...
		if (checkNum !=1){
			bootbox.alert("一次只能对一个批次进行撤销审核");
			return;
		}
	 	modal("#layer_loading,#image");
	 	var rebuyId = getCheckStr("rebuyId");	
	 	dynamicHiddenElement('dataCollectForm','rebuyId',rebuyId);
		dataCollectForm.action = "<%=basePath%>rebuyAuditController.do?method=elecsearchRevokableDetail";
