		...
		<div id="page-content" class="page-content">
				...
				<form  id="btnForm">
					<div class="row-fluid">
						...
						<div class="span6" id="btn-right">
							<a class="btn-mini pull-right" href="javascript:account();">记账</a>
			...
			<div>
				<div id="page" class="pagination">
					<form action="<%=basePath%>rebuyAccountController.do?method=reauditDetailList"  name="pageForm" method="post">
...
<script>
	...
	function goHistory(){
		...
		dataCollectForm.action = "<%=basePath%>rebuyAccountController.do?method=listAccountElec";
	...
	function account(){
		...
		$.ajax({
			url:"<%=basePath%>rebuyAccountController.do?method=checkRiskBills",
			type:"POST",
			data:{"ids":ids},
			dataType:"json",
			success:function(data){
				if(data.success){
					...
					diag.Drag = true;
					diag.Title ="风险票据";
					diag.URL = "<%=basePath%>rebuyAccountController.do?method=toRiskBillsPage&ids="+ids;
					diag.Width = 1200;
					diag.Height = 500;
					diag.CancelEvent = function(){ //关闭事件
					 	submit(ids,rebuyId);
	...
	function submit(ids,rebuyId){
		...
		dataCollectForm.action = "<%=basePath%>rebuyAccountController.do?method=elecaccountSubmit";
		modal("#layer_loading,#image");
		dataCollectForm.submit();
	}
	//打印转贴现记账清单
		...
		$.ajax({
			...
			success:function(rs){
				if(rs.success){//是否计算过利息
				 	doPrint("addPrintController.do?method=doPrint&ids="+ids+"&moduleId=1050101&type=RebuyListPrint&baid="+batch_id+"&handleType=记 账"); 	 
				}else{
