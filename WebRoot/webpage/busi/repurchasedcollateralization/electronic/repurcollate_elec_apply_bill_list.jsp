<%-- 
 * 文件名称: repurcollate_elec_apply_bill_list.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
...
<div class="clearfix">
		<div id="page-content" class="page-content">
			...
			<%-- 按钮操作区 --%>
			<div id="center">
				<form action="<%=basePath%>repurCollateApplyController.do?method=elecApplyBillList"  method="post" id="elecReviewForm" name="elecReviewForm" style="padding:0px;margin-bottom:0px;" class="form-search">
					<div class="row-fluid">
			...
			<div id="page" class="pagination">
					<form action="<%=basePath%>repurCollateApplyController.do?method=elecApplyBillList"  name="pageForm" method="post" >
...
<script>
	...
	function goHistory(){
	    ...
	    dataCollectForm.action="<%=basePath%>repurCollateApplyController.do?method=elecSearchBatch";
	...
	function apply(custNo,custName){
		...
		$.ajax({
			url:"<%=basePath%>repurCollateApplyController.do?method=elecApply",
			type:"POST",
			data:{"ids":ids,"batchNo":batchNo,"batchType":batchType,"batchClass":batchClass,"custNo":custNo,"custManager":custManager,"isTc":isTc},
			dataType:"json",
			success:function(rs){
				if(rs == "yes"){
