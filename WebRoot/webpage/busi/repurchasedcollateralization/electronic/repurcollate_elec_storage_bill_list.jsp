<%-- 
 * 文件名称: repurcollate_elec_storage_bill_list.jsp.jsp
 * 系统名称: 票据管理平台
 * 模块名称:
 * 软件版权: 北京合融科技有限公司
 * 功能说明: 
			...
			<%-- 按钮操作区 --%>
							...
							<c:choose>
								<c:when test="${not empty resultList}">
									<c:forEach items="${resultList}" var="get" varStatus="vs">
										<tr>
											<td class="center"><input type="checkbox" name="ids" value="${get.getmxId}" onclick="getall('ids')" price="${get.billMoney}"/></td>
											...
											<td class="center"><a href="javascript:goBillDetail('${get.rgctId}')">查看</a></td>
										</tr>
									</c:forEach>
								</c:when>
								<c:otherwise>
				...
				<div>
					<div id="page" class="pagination">
							<form action="<%=basePath%>repurCollateStorageController.do?method=elecStorageBillList"  name="pageForm" method="post">
...
<script>
	...
	function goHistory(){
        ...
        dataCollectForm.action="<%=basePath%>repurCollateStorageController.do?method=elecStorageBatchList";
		...
		var checkNum = getCheckNum("ids");
	   	...
	   	bootbox.confirm("确定要出库选中的记录吗?", function(result) {
			if(result) {
				...
				$.ajax({
					type: "POST",
					url: '<%=basePath%>repurCollateStorageController.do?method=elecStorage',
			    	data: {'getmxIds': getmxIds,'getId': getId},
					dataType:'json',
					cache: false,
