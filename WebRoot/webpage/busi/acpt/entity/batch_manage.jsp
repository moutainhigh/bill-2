...
<body>
	<div class="clearfix">
		<div id="page-content" class="page-content">
	    	<form action="<%=basePath%>acptAccountController.do?method=entityAccountList"  method="post" name="dataCollectForm" id="dataCollectForm" class="form-search" >
	   			<%-- 按钮操作区 --%>
				<div id="center">
					<div class="row-fluid">
						<div class="span6" id="btn-left" >
							<a class="btn-mini" href="javascript:elecBillManage();">下一步</a>
					   </div>
							...
							<c:choose>
								<c:when test="${not empty batchList}">
									<c:forEach items="${batchList}" var="batchList" varStatus="vs">
										<tr>
											<td class="center">
												<input type='checkbox' onclick="radioStyle(this,'ids')" name='ids' value="${batchList.acptId}" /><span class="lbl"></span>
											</td>
			...
			<div>
				<div id="page" class="pagination">
					<form action="<%=basePath%>acptAccountController.do?method=entityAccountList"  name="pageForm" method="post">
...
<script>
    ...
    var checkNum = getCheckNum("ids");
   	 if (checkNum !==1){
   		bootbox.alert("一次只能对一个批次进行记账");
   		return;
   	 }
	var acptId = getCheckStr("ids");
		dynamicHiddenElement('dataCollectForm','acptId',acptId);
		modal("#layer_loading,#image");//点击票据管理按钮，弹出‘加载中’的遮罩层
		dataCollectForm.action = "<%=basePath%>acptAccountController.do?method=entitybill";
