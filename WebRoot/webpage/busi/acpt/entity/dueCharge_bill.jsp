...
<body>
	<div class="clearfix">
			...
			<div id="header">
				<form action=""	method="post" name="userForm" id="userForm" class="form-search">
					...
					<div class="row-fluid">
						...
						<input type="text" class="input-medium"  name="issueDt" readonly value="${batch.issueDt}" readonly onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});"/>
						<label for="dueDt">到期日</label>
						<input type="text" class="input-medium"  name="dueDt"  value="${batch.dueDt}" readonly onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});"/>
					</div>
					<div class="row-fluid">
						...
						<input type="text" class="input-medium" name="billType" readonly value="${batch.billType}" readonly/>
					</div>		
				</form>
			</div>
			<%-- 按钮操作区 --%>
			<div id="center">
				<form action="<%=basePath%>acptAccountController.do?method=elecReviewList"  method="post" id="btnForm" name="btnForm" class="form-search">
					<div class="row-fluid">
						<div class="span6" id="btn-left">
							<a class="btn-mini" onclick="doCharge();">到期扣款</a>
							<a class="btn-mini" onclick="doAdvance();">转垫款</a>
					   	</div>
		...
		<div id="page" class="pagination">
			<form action="<%=basePath%>acptAccountController.do?method=dueChargeBill"  name="pageForm" method="post">
...
<script>
	...
	function goHistory(){
		...
		dataCollectForm.action = "<%=basePath%>acptAccountController.do?method=dueCharge";
	 ...
	 function doCharge(){
		bootbox.confirm("确定要扣款选中的记录吗？", function(result) {
	   		if(result) {
		         ...
		         dataCollectForm.action = "<%=basePath%>acptAccountController.do?method=doCharge&acptId="+acptId;
		         dataCollectForm.submit();
	   		  }
	   	});
	}
    function doAdvance(){
		bootbox.confirm("确定要转垫款选中的记录吗？", function(result) {
	   		if(result) {
		         modal("#layer_loading,#image");//点击票据管理按钮，弹出‘加载中’的遮罩层
		         var acptId='${batch.acptId}';
		         dataCollectForm.action = "<%=basePath%>acptAccountController.do?method=doAdvance&acptId="+acptId;
		         dataCollectForm.submit();
	   		  }
	   	});
	}
