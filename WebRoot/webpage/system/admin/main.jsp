...
/* $(document).ready(function(){
	...
	function getContextMenu(){
		$.ajax({
			type: "POST",
			url: '<%=basePath%>UserContextMenuController.do?method=list',
			dataType:'json',
			cache: false,
 ...
 /*调用后台方法删除快捷菜单*/
	...
	function gotoPDeductDrawee(acptId){
		 var url="<%=basePath%>acptAccountController.do?method=dueChargeBill&acptId="+acptId;
		 top.mainFrame.jericho.buildTree("gotoPDeductDrawee","纸票到期扣款提醒",url);
	}
	function gotoEDeductDrawee(acptId){
		 var url="<%=basePath%>acptAccountController.do?method=elecDueChargeBill&acptId="+acptId;
		 top.mainFrame.jericho.buildTree("gotoEDeductDrawee","电票到期扣款提醒",url);
	}
	...
	function gotoESubColl(){
		var url="<%=basePath%>discApplyController.do?method=billManage&discId=2";
	...
	function loadDeductDrawee(){
		$.ajax({
			type: "POST",
			url: "<%=basePath%>remindController.do?method=loadDeductDrawee",
			data:"",
			dataType:'json',
			success: function(data){
	...
	function loadAudit(){
		$.ajax({
			type: "POST",
			url: "<%=basePath%>remindController.do?method=loadAudit",
			data:"",
			dataType:'json',
			success: function(data){	
					$("#AuditDiv").html(data);
	...
	function loadNotice(){
		$.ajax({
			type: "POST",
			url: "<%=basePath%>remindController.do?method=loadNotice",
			data:"",
			dataType:'json',
			success: function(data){	
					$("#NoticeDiv").html(data);
	...
	function loadRdscnt(){
		$.ajax({
			type: "POST",
			url: "<%=basePath%>remindController.do?method=loadDeductRdscnt",
			data:"",
			dataType:'json',
			success: function(data){	
					$("#RdscntDiv").html(data);
